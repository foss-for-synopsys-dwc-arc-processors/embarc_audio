...
{
    return (short)((fx_norm_a64(acc) + 1)&0xfffe);
}
#endif
...
INT16 e)
{
    ...
    i = (q15_t)(in >> 24);
    a = (q31_t)(in & 0x00ffffffL);

    i -= 32;
    val = (q31_t)(sa_sqrt_ldac[i]); 
    dif = (q31_t)(sa_sqrt_ldac[i+1]);
    accum64_t acc = fx_a64_mpy_q31(val, 1 << 24);
    acc = fx_a64_mac_q31(acc, dif, a);
    val =fx_q31_cast_asl_rnd_a64(acc, 23 - (e>>1));
    return (INT32) val;
#endif
}
...
UINT32 nsp)
{
...

    {
        ...
        tmp = (INT32)((acc1 << e) >> 32);
#else 
        acc1 = fx_a64_mac_q31(fx_a64_mpy_q31(y1, y1), y2, y2);
        low_energy = fx_q63_cast_a64(acc1) >> (LDAC_Q_ADD_LOWENERGY + 1);
        e = calc_exp_ldac(acc1);
        tmp = fx_q31_cast_asl_rnd_a64(acc1, e - 1);
#endif
    ...
    for (isp = 1; isp < LDAC_NSP_LOWENERGY; isp++) {
        ...
        tmp = (INT32)((acc1 << e) >> 32);
#else 
        acc1 = fx_a64_mac_q31(fx_a64_mpy_q31(y1, y1), y0-y2, y0-y2);
        low_energy += fx_q63_cast_a64(acc1) >> (LDAC_Q_ADD_LOWENERGY + 1);
        e = calc_exp_ldac(acc1);
        tmp = fx_q31_cast_asl_rnd_a64(acc1, e - 1);
#endif
