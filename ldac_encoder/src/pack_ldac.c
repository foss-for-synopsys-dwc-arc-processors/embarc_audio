...
int *p_loc)
{
    ...
    *p_bufptr = tmp & 0xff;
#else
    assert(24-nbits-bpos>=0);
    /*Here can be problem with loosing data, if we're trying 
    to write too many bits to byte in which we have already
     written something*/
    tmp = (((unsigned int)(idata << (32-nbits))) >> bpos);
    *(unsigned int*)(void*) p_bufptr |= _swape(tmp);
    /*Also there was fixed problem of original code, when this 
    function rewrites two extra bytes beyond the allocated memory. */
#endif
    *p_loc += nbits;
    return;
}
