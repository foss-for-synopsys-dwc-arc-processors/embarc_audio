...
void print_profile_summary(Profiler_Settings_t * inst_settings)
{
    ...
    INFO( "[Memory Usage For Buffers] Maximum Input buffers memory usage: %d bytes",
            inst_settings->profiling_data.max_used_input_buffer);
    INFO( "[Memory Usage For Buffers] Maximum Output buffers memory usage: %d bytes",
			inst_settings->profiling_data.max_used_output_buffer);
...
#if !defined(DCACHE_DISABLED)
    _Uncached int * mss_perfctrl_reg_ctrl = (_Uncached int *)MSS_PERFCTRL_REG_CTRL;
    _Uncached int * mss_perfctrl_reg_lat_ctrl = (_Uncached int *)(MSS_PERFCTRL_REG_LAT_CTRL(MSS_PERFCTRL_MEM_RGON0_IDX));
...
{
#if !defined(DCACHE_DISABLED)
    #if defined MSS_PERFCTRL_MST_FAB_CBU_AXI_IDX
        // Combined Instruction-Bus and Data-Bus Interface (CBU)
        *read_icache = *((uint32_t *)MSS_PERFCTRL_REG_R_COUNT_LO(MSS_PERFCTRL_MST_FAB_CBU_AXI_IDX));
        *write_icache = *((uint32_t *)MSS_PERFCTRL_REG_W_COUNT_LO(MSS_PERFCTRL_MST_FAB_CBU_AXI_IDX));
        *read_dcache = *((uint32_t *)(MSS_PERFCTRL_REG_R_COUNT_LO(MSS_PERFCTRL_MST_FAB_CBU_AXI_IDX) + 0x1000));
        *write_dcache = *((uint32_t *)(MSS_PERFCTRL_REG_W_COUNT_LO(MSS_PERFCTRL_MST_FAB_CBU_AXI_IDX) + 0x1000));
    #elif defined MSS_PERFCTRL_MST_FAB_IBU_AHB_HIDX && defined MSS_PERFCTRL_MST_FAB_DBU_AHB_HIDX
        // EM MSS registers
        // Data-Bus Interface (DBU), Instruction-Bus Interface (IBU)
    ...
    #elif defined MSS_PERFCTRL_MST_FAB_IBU_AXI_IDX && defined MSS_PERFCTRL_MST_FAB_DBU_AXI_IDX
        // HS MSS registers
        // Data-Bus Interface (DBU), Instruction-Bus Interface (IBU)
        *read_icache = *((uint32_t *)MSS_PERFCTRL_REG_R_COUNT_LO(MSS_PERFCTRL_MST_FAB_IBU_AXI_IDX));
        *write_icache = *((uint32_t *)MSS_PERFCTRL_REG_W_COUNT_LO(MSS_PERFCTRL_MST_FAB_IBU_AXI_IDX));
        *read_dcache = *((uint32_t *)MSS_PERFCTRL_REG_R_COUNT_LO(MSS_PERFCTRL_MST_FAB_DBU_AXI_IDX));
        *write_dcache = *((uint32_t *)MSS_PERFCTRL_REG_W_COUNT_LO(MSS_PERFCTRL_MST_FAB_DBU_AXI_IDX));
    #else
    #error "No cache registers found!"
    #endif
#endif // !defined(DCACHE_DISABLED)
}
#endif
